...
export function ImageWorkbench() {
  ...
  function appendReferenceMention(reference: ReferenceImage) { const label = getReferenceMentionLabel(references.findIndex((item) => item.id === reference.id)); updatePrompt(`${promptText}${promptText.endsWith(' ') || !promptText ? '' : ' '}${label} `); }
...
function ImagePromptEditor({ mentionedReferences, onChange, references, value }: { value: string; onChange: (value: string) => void; references: ReferenceImage[]; mentionedReferences: ReferenceImage[] }) {
  ...
  useEffect(() => { const root = editorRef.current; if (!root || composingRef.current || document.activeElement === root || editorDomToText(root) === value) return; renderEditorText(root, value); }, [value]);

  function syncFromDom() { const root = editorRef.current; if (!root) return; onChange(editorDomToText(root)); }
  function insertReference(reference: ReferenceImage) { const root = editorRef.current; if (!root) return; root.focus(); const index = references.findIndex((item) => item.id === reference.id); insertMentionToken(root, getReferenceMentionLabel(index), reference.id); setMenuOpen(false); onChange(editorDomToText(root)); }
...
function renderEditorText(root: HTMLDivElement, value: string) { root.textContent = ''; for (const part of value.split(/(@\u56fe\d+)/g)) { if (!part) continue; if (/^@\u56fe\d+$/.test(part)) { const token = document.createElement('span'); token.className = 'image-studio__prompt-mention-token'; token.contentEditable = 'false'; token.dataset.token = part; token.textContent = part; root.appendChild(token); } else { root.appendChild(document.createTextNode(part)); } } }
