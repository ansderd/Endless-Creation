...
export function ImageWorkbench() {
  ...
  function handleReferenceUpload(event: ChangeEvent<HTMLInputElement>) { const files = Array.from(event.currentTarget.files ?? []); event.currentTarget.value = ''; if (!files.length) return; const accepted: ReferenceImage[] = []; let message = ''; setReferences((current) => { const available = MAX_REFERENCE_IMAGES - current.length; if (available <= 0) { message = '参考图最多添加 4 张。'; return current; } for (const file of files) { if (accepted.length >= available) { message = '参考图最多添加 4 张，已忽略多余图片。'; break; } if (!file.type.startsWith('image/')) { message = `${file.name} 不是图片文件。`; continue; } if (file.size > MAX_REFERENCE_IMAGE_SIZE) { message = `${file.name} 超过 8MB。`; continue; } const previewUrl = URL.createObjectURL(file); uploadObjectUrlsRef.current.push(previewUrl); const nextIndex = current.length + accepted.length + 1; accepted.push({ id: createId('ref'), name: file.name || `参考图 ${nextIndex}`, source: 'upload', previewLabel: file.name || `参考图 ${nextIndex}`, palette: palettes[(nextIndex - 1) % palettes.length], previewUrl, size: file.size }); } return accepted.length ? [...current, ...accepted] : current; }); setFeedback(accepted.length ? `已上传 ${accepted.length} 张参考图。` : message || '未选择可用图片。'); if (status === 'idle' && accepted.length) setStatus('editing'); }
